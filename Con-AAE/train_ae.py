...
#genomics_dataset = RNA_Dataset(datadir="./data/"+args.input_rna,labeldir="./data/"+args.input_label,mode='train')
#ATAC_dataset = ATAC_Dataset(datadir="./data/"+args.input_atac,labeldir="./data/"+args.input_label,mode='train')
shuffle=False
...
#input('pause')
if args.VAE:
  netRNA=FC_VAE(n_input=genomics_dim,n_hidden=genomics_dim,nz=args.nz)
  netATAC=FC_VAE(n_input=ATAC_dim,n_hidden=ATAC_dim,nz=args.nz)
else:
  netRNA = FC_Autoencoder(n_input=genomics_dim, nz=args.nz,n_hidden=genomics_dim)
  netATAC = FC_Autoencoder(n_input=ATAC_dim, nz=args.nz,n_hidden=ATAC_dim)

print("Pre-trained model loaded from %s" % args.pretrained_file)
if args.conditional:
    netCondClf = Simple_Classifier(nz=args.nz,n_out=label_num)
    
if args.discriminator:
    netClf=FC_Classifier(nz=args.nz,n_out=1)
...
def train_autoencoders(rna_inputs, atac_inputs, rna_class_labels=None, atac_class_labels=None):
    ...
    if args.discriminator:
        ...
        #print(type(rna_scores))
        #print(type(rna_lab  els))
        #input('pause')
...
for epoch in range(args.max_epochs):
    ...
    #print('Epoch: ', epoch, ', clf loss: %.8f' % float(clf_loss/cell_num))
